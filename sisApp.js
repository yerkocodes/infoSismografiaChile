  ...
  .controller('SismoAppController', function($scope, $http) {
    ...
    $scope.jsonComunas = JSON.stringify({
      "regiones": [
        ...
        {
          "region": "Coquimbo",
          "comunas": ["La Serena", "Coquimbo", "Tongoy", "Andacollo", "La Higuera", "Paiguano", "Paihuano", "Vicuña", "Illapel", "Canela", "Los Vilos", "Salamanca", "Ovalle", "Combarbalá", "Monte Patria", "Punitaqui", "Río Hurtado"]
        },
        {
          "region": "Valparaíso",
    ...
    $scope.returnCompleteAddress = function(localidad, regionesArr) {
      ...
      for(i = 0; i < regionesArr.length; i++) {
        for (let f = 0; f < regionesArr[i].comunas.length; f++) {
          if( regionesArr[i].comunas[f].toLowerCase() == localidad.toLowerCase() ) {
            result = `${regionesArr[i].comunas[f]}, ${regionesArr[i].region}`;
            break;
          }
        }
